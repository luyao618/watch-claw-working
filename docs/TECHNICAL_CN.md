...
| **Bridge WebSocket** | 连接被拒绝（Bridge 未启动） | 指数退避自动重连；保持 'connecting' 状态 | Badge 显示 "Connecting..."，角色保持 idle |
...
- **Debug grid overlay**：切换 grid 线条用于 tile 对齐
- **FPS 计数器**：显示当前帧率
...
### 10.1 移除 Mock Provider

v0.2 中完全移除 `MockProvider` 类及所有 mock 回退逻辑。`ConnectionManager` 不再有 `'mock'` 状态。当 Bridge Server 不可用时，系统保持 `'connecting'` 状态并指数退避重试，角色保持 idle/sleeping。永不生成假数据。
...
**修改后的 ConnectionManager 状态机：**

```
DISCONNECTED → CONNECTING → CONNECTED ('live')
                    ↑            ↓ (断开)
                    └── RECONNECTING (指数退避, 状态保持 'connecting')
```

### 10.2 渲染方案：Isometric → 3/4 俯视角

渲染引擎从 isometric（2:1 菱形 tiles）切换到 3/4 俯视角（星露谷物语风格矩形 tiles）。

**关键变更：**

- `isometric.ts` → `coordinates.ts` — 简单的笛卡尔到屏幕乘法替代菱形数学
- Tile 形状：矩形（如 32x32 或 16x16）替代 64x32 菱形
- 深度排序：基于 Y 坐标（Y 越小越远）替代 col+row
- Sprites 使用 `ctx.drawImage()` 从 spritesheet 渲染，替代程序化的 `ctx.fillRect()`/`ctx.beginPath()`
- Camera 简化为 top-down 平移/缩放（无 isometric 偏移数学）

### 10.3 房间命名

| v0.1        | v0.2              | RoomId       |
| ----------- | ----------------- | ------------ |
| Office      | 工作室 (Workshop) | `'workshop'` |
| Living Room | 书房 (Study)      | `'study'`    |
| Bedroom     | 卧室 (Bedroom)    | `'bedroom'`  |

### 10.4 Sprite-Based 渲染

v0.2 将所有程序化绘制（彩色矩形、几何图形）替换为 sprite-based 渲染：

- **Tiles**：从 tileset PNG 加载，切割为单个 tile 帧
- **家具**：每个家具项目独立 sprite PNG
- **角色**：Spritesheet，包含动画帧（walk/idle/type/sleep/sit）
- **情绪**：Sprite-based 情绪气泡

回退机制：如果 sprite 文件缺失，渲染彩色占位方块并输出 console 警告。

### 10.5 Electron 桌面应用

v0.2 将 Web 应用包装在 Electron shell 中，作为独立桌面应用发布。
...
│                                                                 │
│  ┌─────────────────┐  ┌─────────────────────────────────────┐  │
│  │  BrowserWindow   │  │  Bridge Server (fork 子进程)        │  │
│  │  (加载渲染进程    │  │  - fs.watch session JSONL            │  │
│  │   via Vite 或    │  │  - WebSocket server :18790           │  │
│  │   打包文件)      │  │  - Session 发现/切换                 │  │
│  └────────┬─────────┘  └────────────────────────┬────────────┘  │
│           │ IPC                                  │               │
│  ┌────────▼─────────────────────────────────────▼────────────┐  │
...
- Bridge Server 由 Electron 主进程 fork 启动，统一生命周期管理
- 窗口位置/大小跨会话记忆
- 置顶（Always-on-top）可通过托盘菜单切换
- 关闭窗口隐藏到托盘（不退出）；通过托盘菜单退出
...
- `pnpm dev:electron` — 同时启动 Vite + Bridge Server + Electron
- `pnpm build:electron` — 打包为 macOS .app / .dmg
