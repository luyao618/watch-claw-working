...
| 项目                                                              | 我们借鉴了什么                                | 我们做了哪些不同                                                                                                  |
| ----------------------------------------------------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| [Pixel Agents](https://github.com/pablodelucca/pixel-agents)      | JSONL 文件监听、角色 FSM、Canvas 2D rendering | 我们使用 Bridge Server 推送（而非直接 file tailing）、isometric 视角（而非 top-down）、单角色（而非 multi-agent） |
...
| 房间            | Agent 活动                                                 | 角色动画               | 情绪     |
| --------------- | ---------------------------------------------------------- | ---------------------- | -------- |
| **Office**      | `write`、`edit`、`exec`、assistant text streaming          | 坐在桌前，在键盘上打字 | Focused  |
| **Living Room** | `read`、`grep`、`glob`、`web_search`、思考（无 tool call） | 坐在沙发上，看着壁炉   | Thinking |
...
| 4   | **Mock mode**          | 当 Bridge Server 不可用时，生成模拟事件用于开发和演示                                            |
...
| --------------------------------- | ---------------- | --------------------------------------- | ----------------- | --------- | ------ |
| `type: "session"`（session 开始） | --               | Living Room                             | 起床，走到沙发    | Thinking  | High   |
| `stopReason: "stop"`（会话结束）  | --               | Bedroom                                 | 走到床边，躺下    | Sleepy    | High   |
...
| `tool: exec`                      | exec             | Office (MVP) / Tool Room (v1.0)         | 打字 / 使用工具   | Serious   | Medium |
...
| `tool: web_search`                | web_search       | Living Room                             | 在平板/手机上浏览 | Curious   | Medium |
...
| **Mock mode 回退** | < 100ms 切换到 mock（当 Bridge Server 不可达时）     |
| **内存使用**       | < 100MB 浏览器内存占用                               |
