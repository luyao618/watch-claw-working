...
**Watch Claw** 是一个实时 pixel-art 可视化项目，用于展示 [OpenClaw](https://github.com/openclaw/openclaw) AI agent 的工作状态。它渲染一个等轴测（isometric）剖面视角的温馨三层小屋，一个戴着龙虾帽的角色——代表 OpenClaw agent——会在各个房间之间移动、执行活动，并根据 agent 的实际运行状态表达情绪。

项目通过 WebSocket（`ws://127.0.0.1:18789`）直接连接 OpenClaw Gateway，解析实时 agent 事件（tool calls、lifecycle phases、presence），并将其转化为角色行为：走到办公室写代码、坐在沙发上思考、空闲时在床上睡觉。
...
| 项目 | 我们借鉴了什么 | 我们做了哪些不同 |
|---------|---------------|----------------------|
| [Pixel Agents](https://github.com/pablodelucca/pixel-agents) | JSONL 文件监听、角色 FSM、Canvas 2D rendering | 我们使用 WebSocket（而非 file tailing）、isometric 视角（而非 top-down）、单角色（而非 multi-agent） |
| [PixelHQ ULTRA](https://github.com/RemyLoveLogicAI/pixelhq-ultra) | Event-driven 架构、personality engine | 我们专注于温馨小屋（而非企业办公室）、高精度 pixel art（而非 DOM tiles） |
...
```
OpenClaw 做了什么 → 事件通过 WebSocket 到达 → 角色移动到对应房间 → 播放动画
```
...
| 房间 | Agent 活动 | 角色动画 | 情绪 |
|------|---------------|--------------------|---------| 
| **Office** | `Write`、`Edit`、`Bash`、assistant text streaming | 坐在桌前，在键盘上打字 | Focused |
| **Living Room** | `Read`、`Grep`、`Glob`、`WebFetch`、思考（无 tool call） | 坐在沙发上，看着壁炉 | Thinking |
| **Bedroom** | Idle（无活动）、等待用户输入、session 结束 | 躺在床上，睡觉 | Sleepy |
...
| # | 功能 | 描述 |
|---|---------|-------------|
| 1 | **WebSocket 连接** | 连接到 OpenClaw Gateway（`ws://127.0.0.1:18789`），处理 handshake，自动重连 |
| 2 | **Event 解析** | 解析 `agent` events（lifecycle/tool/assistant streams）、`presence`、`health` |
| 3 | **Event 到行为映射** | 将解析后的 events 映射为角色动作（去某个房间、播放动画、显示情绪） |
| 4 | **Mock mode** | 当 Gateway 不可用时，生成模拟事件用于开发和演示 |
| 5 | **Isometric renderer** | Canvas 2D isometric tile rendering，使用正确的绘制顺序（painter's algorithm） |
| 6 | **单层楼布局** | 主楼层包含 3 个房间：office、living room、bedroom |
| 7 | **龙虾帽角色** | 带动画的 pixel 角色，状态包括：idle、walk、sit、type、sleep |
| 8 | **Pathfinding** | 在 tile grid 上进行 BFS pathfinding，角色在房间之间平滑移动 |
| 9 | **情绪气泡** | 在角色头顶显示情绪：focused、thinking、sleepy、happy、confused |
| 10 | **状态面板** | 侧边栏显示：连接状态、当前 agent 状态、token 使用量、session 信息 |
...
| 房间 | Agent 活动 | 角色动画 | 情绪 |
|------|---------------|--------------------|---------| 
| **Office** | `Write`、`Edit`、`Bash`、assistant streaming | 在桌前打字 | Focused |
| **Living Room** | `WebFetch`、思考、轻度浏览 | 坐在沙发上 | Thinking |
...
| 房间 | Agent 活动 | 角色动画 | 情绪 |
|------|---------------|--------------------|---------| 
| **Tool Room** | `Bash`（系统命令、脚本） | 在工作台上使用扳手/锤子 | Serious |
...
| Gateway Event | `event` 字段 | `stream` / 关键字段 | 可用数据 |
|--------------|--------------|---------------------|----------------|
| Agent lifecycle start | `agent` | `stream: "lifecycle"`, `phase: "start"` | `runId` |
| Agent lifecycle end | `agent` | `stream: "lifecycle"`, `phase: "end"` | `runId`, usage 统计 |
| Agent lifecycle error | `agent` | `stream: "lifecycle"`, `phase: "error"` | 错误详情 |
| Tool call | `agent` | `stream: "tool"` | Tool name、params、result |
| Assistant text | `agent` | `stream: "assistant"` | Text delta（streaming） |
| Presence update | `presence` | Device entries | 已连接客户端、最后输入 |
| Health check | `health` | Health snapshot | 系统状态 |

### 6.2 完整映射表

| OpenClaw Event | Tool / Phase | 目标房间 | 动画 | 情绪 | 优先级 |
|---------------|-------------|-------------|-----------|---------|----------|
| `lifecycle.start` | -- | Living Room | 起床，走到沙发 | Thinking | High |
| `lifecycle.end` | -- | Bedroom | 走到床边，躺下 | Sleepy | High |
| `lifecycle.error` | -- | （当前房间） | 坐下，抱头 | Confused | High |
| `tool: Write` | Write | Office | 在键盘上打字 | Focused | Medium |
| `tool: Edit` | Edit | Office | 在键盘上打字 | Focused | Medium |
| `tool: Bash` | Bash | Office (MVP) / Tool Room (v1.0) | 打字 / 使用工具 | Serious | Medium |
| `tool: Read` | Read | Living Room (MVP) / Reading Room (v1.0) | 阅读，翻页 | Curious | Medium |
| `tool: Grep` | Grep | Living Room (MVP) / Reading Room (v1.0) | 在书中搜索 | Curious | Medium |
| `tool: Glob` | Glob | Living Room (MVP) / Storage Room (v1.0) | 浏览书架 | Busy | Medium |
| `tool: WebFetch` | WebFetch | Living Room | 在平板/手机上浏览 | Curious | Medium |
| `tool: Task` | Task (sub-agent) | Living Room (MVP) / Lab (v1.0) | 在白板上画画 | Excited | Medium |
| `assistant` streaming | -- | Office | 在键盘上打字 | Focused | Low |
| 无事件（idle > 30s） | -- | Bedroom | 睡觉 | Sleepy | Low |
| 任务完成 | -- | Living Room (MVP) / Balcony (v1.0) | 庆祝，伸展 | Satisfied | Medium |

### 6.3 优先级解析

当多个事件快速连续到达时，高优先级事件优先处理。优先级分为：

- **High**：Lifecycle events（`start`、`end`、`error`）—— 覆盖一切
...
| **启动时间** | < 2s 到首次有意义的渲染 |
| **WebSocket 重连** | 带指数退避的自动重连（1s、2s、4s、... 最大 30s） |
| **Mock mode 回退** | < 100ms 切换到 mock（当 Gateway 不可达时） |
| **内存使用** | < 100MB 浏览器内存占用 |
